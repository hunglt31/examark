...
TritonClient::TritonClient(const std::string& url)
  : m_url(url) {
}
...
std::vector<std::vector<Detection>> TritonClient::inference(
  ...
  const std::string& modelName) 
{
  ...
  try {
    ...
    outputs.push_back(output1_ptr.get());
    outputs.push_back(output2_ptr.get());
    outputs.push_back(output3_ptr.get());
...
std::vector<std::vector<Detection>> TritonClient::postprocess(
    const std::vector<cv::Mat>& images, 
    ...
    try {
        ...
        for (int i = 0; i < numImages; i++) {
            ...
            for (int j = 0; j < maxOutputBoxes; j++) {
                ...
                det.box = cv::Rect(cvRound(x1), cvRound(y1), cvRound(x2 - x1), cvRound(y2 - y1));
                det.score = scores[detIdx];
                det.classId = classes[detIdx];

                cv::Rect safeBox = det.box & cv::Rect(0, 0, gray.cols, gray.rows);
                cv::Scalar meanVal = cv::mean(gray(safeBox));
                det.avgGray = meanVal[0];
                
                results[i].push_back(det);
            }
        }
        return results;
    }
...
std::string TritonClient::getCurrentTimestamp() {
    ...
    std::stringstream ss;
    ss << std::put_time(std::localtime(&time_t_now), "%Y%m%d_%H%M%S") << "_" << std::setw(3) << std::setfill('0') << ms.count();
    return ss.str();
}

void TritonClient::drawAndSaveResults(
    const std::vector<cv::Mat>& images, 
    ...
    const std::string& outputDir)
{
    ...
    for (size_t i = 0; i < images.size(); ++i) {
        ...
        for (const auto& det : detections[i]) {
            ...
            if (det.classId == 1) color = cv::Scalar(0, 0, 255); 
            // Draw bounding box

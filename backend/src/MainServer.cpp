...
//   Logger::warning("SERVER", "Interrupt signal (" + std::to_string(signum) +
...
bool initializeModels() {
  ...
  g_metadataModel = std::make_unique<ModelBuilder>(
      METADATA_MODEL_PATH, METADATA_ENGINE_PATH, INPUT_WIDTH, INPUT_HEIGHT,
      METADATA_BATCH_SIZE, METADATA_TOP_K, METADATA_MAX_OUTPUT_BOXES);

  g_contentModel = std::make_unique<ModelBuilder>(
      CONTENT_MODEL_PATH, CONTENT_ENGINE_PATH, INPUT_WIDTH, INPUT_HEIGHT,
...
int main() {
  ...
  registerGradingRouteTRT(examarkServer, g_metadataModel.get(),
                          g_contentModel.get());

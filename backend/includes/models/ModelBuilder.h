...
public:
  /**
   * @brief Constructs the engine with specified ONNX model path, engine path,
   ...
   * @param maxOutputBoxes The maximum number of boxes to keep after applying
   * NMS.
   */
  explicit ModelBuilder(const std::string &modelPath,
                        const std::string &enginePath, int inputWidth,
                        int inputHeight, int batchSize, int topK,
                        int maxOutputBoxes);
   ...
   *
   * @return true if the engine is successfully loaded or built; false
   * otherwise.
   */
  ...
  inference(const std::vector<cv::Mat> &originImgs);
